<!--
 ...
 -->
<script lang="ts">
  type Event = $$Generic<{ detail: { e: MouseEvent } }>;

  interface $$Slots {
    default: { detail: Event['detail'] };
  }

  /** A svelte event created via [`dispatch`](https://svelte.dev/docs#createEventDispatcher) with event information under `evt.detail.e`. */
  export let evt: Event | null | undefined;
